...
n_curves <-  2000                    # number of lines to draw
...
curve_stroke <-   0.8                # stroke width of lines
curve_alpha <-  0.7                  # alpha of lines

# Create matrix with nx * ny points, values are angle with Perlin noise
pnt <- array(noise_worley(c(nx, ny)), dim = c(nx, ny), dimnames = list(1:nx, 1:ny)) * pi * 3.3
...
# Plot
pal <- colorRampPalette(c('#003326','#005150','#006F80','#3E8CB2','#7EA7DE'))
pal2 <- colorRampPalette(c('#003326','#84B5A4','#7C84FF','#0B3CAB'))

ggplot(dat_df) +
    geom_segment(aes(x = x_start, y = y_start, xend = x_end, yend = y_end, color = i),
                 size = curve_stroke, alpha = curve_alpha,
                 lineend = "butt", linejoin = "mitre") + 
    scale_color_gradientn(colors = pal2(20)) + 
    coord_fixed(xlim = c(limit * 1, nx - limit * 1), ylim = c(limit * 1, ny - limit * 1), expand = FALSE) + 
    theme_void() +
    theme(
        legend.position = "none",
        plot.background = element_rect(fill = "#00140f")
    ) 
#ggsave('geomoss2.png', height = 10, width = 10, units = 'in', dpi = 1200)
